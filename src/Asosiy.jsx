...
import { Link } from "react-router-dom" // Next.js Link ni React Router Link bilan almashtirdim

const Asosiy = () => {
  const [activeTab, setActiveTab] = useState("Backend")
  const [qoraTema, setQoraTema] = useState(true)
  const [showVideoModal, setShowVideoModal] = useState(false) // Video modal uchun state qo'shdim
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${qoraTema ? "bg-white" : "bg-gray-900"}`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 max-w-4xl w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">iTLive Video</h3>
      ...
      <button
        ...
        className={`fixed top-20 right-4 z-50 p-3 rounded-full transition-all duration-300 hover:scale-110 ${qoraTema ? "bg-yellow-300 text-gray-900 hover:bg-yellow-200" : "bg-gray-800 text-white hover:bg-gray-700"
          }`}
      >
        {qoraTema ? "☀️" : "🌙"}
      </button>
      ...
      <header
        className={`shadow-sm px-4 lg:px-8 py-4 fixed top-0 w-full z-50 transition-colors duration-300 ${qoraTema ? "bg-gray-800 border-gray-700" : "bg-white"
          }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/login"
              className={`px-4 py-2 hover:animate-pulse transition-colors bg-blue-500 rounded-2xl ${qoraTema ? "text-gray-300" : "text-gray-50"
                }`}
            >
              Login
            </Link>
      ...
      {/* Hero Section */}
      <section
        className={`pt-24 pb-16 transition-colors duration-300 ${qoraTema ? "bg-gradient-to-br from-gray-800 to-gray-900" : "bg-gradient-to-br from-blue-50 to-indigo-50"
          }`}
      >
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <h1
                className={`text-4xl lg:text-5xl font-bold leading-tight ${qoraTema ? "text-white" : "text-gray-900"}`}
              >
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-12">
            {tabs.map((tab) => (
              <button
                key={tab}
                className={`px-4 py-2 rounded-full text-sm transition-colors ${activeTab === tab
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {courses.map((course, index) => (
              <div
                key={index}
                className={`rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow ${qoraTema ? "bg-gray-800" : "bg-white"
                  }`}
              >
                <div className={`${course.color} p-6 text-white relative`}>
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="inline-block px-3 py-1 bg-white/20 rounded-full text-sm mb-3">{course.level}</div>
                ...
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center ${qoraTema ? "bg-gray-700" : "bg-gray-200"
                        }`}
                    >
                      👨
                    </div>
                  ...
                  <div className="mb-6">
                    <div
                      className={`flex justify-between text-sm mb-2 ${qoraTema ? "text-gray-400" : "text-gray-600"}`}
                    >
                      <span>Davomiyligi: {course.duration}</span>
                      <span>{course.progress}%</span>
                    </div>
                    <div className={`w-full rounded-full h-2 ${qoraTema ? "bg-gray-700" : "bg-gray-200"}`}>
                      <div
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      <div className={`text-sm line-through ${qoraTema ? "text-gray-500" : "text-gray-400"}`}>
                        {course.oldPrice} som
                      </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-12 max-w-4xl mx-auto">
            ...
            <div className="text-center">
              ...
              <button
                className={`px-6 py-3 border-2 border-blue-600 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-colors ${qoraTema ? "hover:text-white" : ""
                  }`}
              >
                Mentor
              </button>
                ...
                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end">
                  <div className="p-4 text-center">
                    ...
                    <div className="flex justify-center gap-4 mt-3 text-white">
                      <a href="#">
                        <i className="fab fa-telegram text-xl hover:text-blue-400"></i>
                      </a>
                      <a href="#">
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {reviews.map((review, index) => (
              <div
                key={index}
                className={`p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow ${qoraTema ? "bg-gray-700" : "bg-white"
                  }`}
              >
                <div className="flex items-center gap-3 mb-4">
                  <img
      ...
      {/* Footer */}
      <footer
        className={`py-12 border-t transition-colors duration-300 ${qoraTema ? "bg-gray-900 border-gray-700" : "bg-white border-gray-200"
          }`}
      >
        <div className="max-w-7xl mx-auto px-4 text-center">
